const express = require('express')
const path = require('path')
const fs = require('fs')

const router = express.Router()

const DATA_DIR = path.join(__dirname, '..', 'data')
const DATA_FILE = path.join(DATA_DIR, 'resenas.json')
const JUEGOS_FILE = path.join(DATA_DIR, 'juegos.json')
const FRONTEND_DIR = path.join(__dirname, '..', 'frontend', 'end-points')

function ensureStorage() {
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true })
  if (!fs.existsSync(DATA_FILE)) fs.writeFileSync(DATA_FILE, '[]', 'utf-8')
  if (!fs.existsSync(JUEGOS_FILE)) fs.writeFileSync(JUEGOS_FILE, '[]', 'utf-8')
}

function readReviews() {
  ensureStorage()
  try {
    const raw = fs.readFileSync(DATA_FILE, 'utf-8')
    return JSON.parse(raw)
  } catch (e) {
    fs.writeFileSync(DATA_FILE, '[]', 'utf-8')
    return []
  }
}

function writeReviews(list) {
  ensureStorage()
  fs.writeFileSync(DATA_FILE, JSON.stringify(list, null, 2), 'utf-8')
}

function readGames() {
  ensureStorage()
  try {
    const raw = fs.readFileSync(JUEGOS_FILE, 'utf-8')
    const data = JSON.parse(raw)
    if (Array.isArray(data) && Array.isArray(data[0])) return data[0]
    return data
  } catch (e) {
    fs.writeFileSync(JUEGOS_FILE, '[]', 'utf-8')
    return []
  }
}

function writeGames(list) {
  ensureStorage()
  fs.writeFileSync(JUEGOS_FILE, JSON.stringify(list, null, 2), 'utf-8')
}

router.get('/api/resenas', (req, res) => {
  const reseñas = readReviews()
  res.json(reseñas)
})

router.post('/api/resenas', (req, res) => {
  const body = req.body || {}
  const { usuario, titulo, imagen, texto, level, fecha, calificacion } = body
  if (!titulo || !texto || !level) {
    return res.status(400).json({ error: 'Faltan campos requeridos: titulo, texto, level' })
  }
  const reseñas = readReviews()
  const nueva = {
    id: Date.now(),
    usuario: usuario && String(usuario).trim() ? usuario : 'Invitado',
    titulo,
    imagen: imagen || 'img/placeholder.jpg',
    texto,
    level,
    fecha: fecha || new Date().toISOString(),
    calificacion: calificacion !== undefined ? Number(calificacion) || 0 : 0,
    likes: Number(body.likes) || 0,
  }
  reseñas.push(nueva)
  writeReviews(reseñas)
  res.status(201).json(nueva)
})

router.get('/api/resenas/:id', (req, res) => {
  const { id } = req.params
  const reseñas = readReviews()
  const item = reseñas.find(r => String(r.id) === String(id))
  if (!item) return res.status(404).json({ error: 'Reseña no encontrada' })
  res.json(item)
})

router.put('/api/resenas/:id/likes', (req, res) => {
  const { id } = req.params
  const { likes } = req.body || {}
  const reseñas = readReviews()
  const idx = reseñas.findIndex(r => String(r.id) === String(id))
  if (idx === -1) return res.status(404).json({ error: 'Reseña no encontrada' })
  reseñas[idx].likes = Number(likes) || 0
  writeReviews(reseñas)
  res.json({ id: reseñas[idx].id, likes: reseñas[idx].likes })
})

router.put('/api/resenas/:id', (req, res) => {
  const { id } = req.params
  const { titulo, texto, imagen, calificacion, level } = req.body || {}
  if (!titulo && !texto && !level) {
    return res.status(400).json({ error: 'Faltan campos requeridos: titulo, texto, level' })
  }
  const reseñas = readReviews()
  const idx = reseñas.findIndex(r => String(r.id) === String(id))
  if (idx === -1) return res.status(404).json({ error: 'Reseña no encontrada' })
  if (typeof titulo === 'string') reseñas[idx].titulo = titulo
  if (typeof texto === 'string') reseñas[idx].texto = texto
  if (typeof imagen === 'string' && imagen.length) reseñas[idx].imagen = imagen
  if (calificacion !== undefined) reseñas[idx].calificacion = Number(calificacion) || 0
  if (typeof level === 'string') reseñas[idx].level = level
  writeReviews(reseñas)
  res.json(reseñas[idx])
})

router.delete('/api/resenas/:id', (req, res) => {
  const { id } = req.params
  const reseñas = readReviews()
  const idx = reseñas.findIndex(r => String(r.id) === String(id))
  if (idx === -1) return res.status(404).json({ error: 'Reseña no encontrada' })
  const [eliminada] = reseñas.splice(idx, 1)
  writeReviews(reseñas)
  res.json({ ok: true, id: eliminada.id })
})

router.get('/health', (req, res) => {
  res.json({ ok: true })
})

router.get('/api/juegos', (req, res) => {
  const juegos = readGames()
  res.json(juegos)
})

router.get('/api/juegos/:id', (req, res) => {
  const { id } = req.params
  const juegos = readGames()
  const juego = juegos.find(j => String(j.id) === String(id))
  if (!juego) return res.status(404).json({ error: 'Juego no encontrado' })
  res.json(juego)
})

router.post('/api/juegos', (req, res) => {
  const body = req.body || {}
  const { titulo, cover, generos, descripcion, plataformas } = body
  if (!titulo) return res.status(400).json({ error: 'Falta titulo' })
  const juegos = readGames()
  const nuevo = {
    id: Date.now(),
    titulo,
    cover: cover || '',
    generos: Array.isArray(generos) ? generos : [],
    descripcion: descripcion || '',
    plataformas: Array.isArray(plataformas) ? plataformas : [],
  }
  juegos.push(nuevo)
  writeGames(juegos)
  res.status(201).json(nuevo)
})

router.put('/api/juegos/:id', (req, res) => {
  const { id } = req.params
  const { titulo, cover, generos, descripcion, plataformas } = req.body || {}
  const juegos = readGames()
  const idx = juegos.findIndex(j => String(j.id) === String(id))
  if (idx === -1) return res.status(404).json({ error: 'Juego no encontrado' })
  if (typeof titulo === 'string') juegos[idx].titulo = titulo
  if (typeof cover === 'string') juegos[idx].cover = cover
  if (Array.isArray(generos)) juegos[idx].generos = generos
  if (typeof descripcion === 'string') juegos[idx].descripcion = descripcion
  if (Array.isArray(plataformas)) juegos[idx].plataformas = plataformas
  writeGames(juegos)
  res.json(juegos[idx])
})

router.delete('/api/juegos/:id', (req, res) => {
  const { id } = req.params
  const juegos = readGames()
  const idx = juegos.findIndex(j => String(j.id) === String(id))
  if (idx === -1) return res.status(404).json({ error: 'Juego no encontrado' })
  const [eliminado] = juegos.splice(idx, 1)
  writeGames(juegos)
  res.json({ ok: true, id: eliminado.id })
})

router.get('/', (req, res) => {
  res.sendFile(path.join(FRONTEND_DIR, 'index.html'))
})

router.get('/inicio', (req, res) => {
  res.sendFile(path.join(FRONTEND_DIR, 'src', 'pages', 'inicio', 'inicio.html'))
})

router.get('/biblioteca', (req, res) => {
  res.sendFile(path.join(FRONTEND_DIR, 'src', 'pages', 'Biblioteca', 'biblioteca.html'))
})

router.get('/reseñas', (req, res) => {
  res.sendFile(path.join(FRONTEND_DIR, 'src', 'pages', 'reseñas', 'reseñas.html'))
})

router.get('/resenas', (req, res) => {
  res.sendFile(path.join(FRONTEND_DIR, 'src', 'pages', 'reseñas', 'reseñas.html'))
})

router.use(express.static(FRONTEND_DIR))

module.exports = router